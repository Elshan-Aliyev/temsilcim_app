...
const UpdateProperty = () => {
    ...
    const fetchProperty = async () => {
      try {
        ...
        if (property) {
          ...
          setBuildingName(property.location?.buildingName || '');
          setFloorNumber(property.location?.floorNumber?.toString() || '');
          setUnitNumber(property.location?.unitNumber || '');
          setLatitude(property.location?.coordinates?.latitude?.toString() || '');
          setLongitude(property.location?.coordinates?.longitude?.toString() || '');
          setNearbyLandmarks(property.location?.nearbyLandmarks?.join(', ') || '');

          // Pricing
          setPrice(property.pricing?.price?.toString() || '');
          setNegotiable(property.pricing?.negotiable || false);
          setCurrency(property.pricing?.currency || 'AZN');
          setMonthlyRent(property.pricing?.monthlyRent?.toString() || '');
          setDepositAmount(property.pricing?.depositAmount?.toString() || '');
          setPaymentFrequency(property.pricing?.paymentFrequency || '');
          setUtilitiesIncluded(property.pricing?.utilitiesIncluded || false);
          setMinimumContractPeriod(property.pricing?.minimumContractPeriod?.toString() || '');

          // Specifications
          setBuiltUpArea(property.specifications?.builtUpArea?.toString() || '');
          setLandArea(property.specifications?.landArea?.toString() || '');
          setFloorArea(property.specifications?.floorArea?.toString() || '');
          setPlotWidth(property.specifications?.plotWidth?.toString() || '');
          setPlotLength(property.specifications?.plotLength?.toString() || '');
          setYearBuilt(property.specifications?.yearBuilt?.toString() || '');
          setRenovationYear(property.specifications?.renovationYear?.toString() || '');
          setConstructionStatus(property.specifications?.constructionStatus || '');
          setTotalFloorsInBuilding(property.specifications?.totalFloorsInBuilding?.toString() || '');
          setFloorNumberOfUnit(property.specifications?.floorNumberOfUnit?.toString() || '');

          // Rooms
          setBedrooms(property.rooms?.bedrooms?.toString() || '');
          setBathrooms(property.rooms?.bathrooms?.toString() || '');
          setBalconies(property.rooms?.balconies?.toString() || '');
          setMaidsRoom(property.rooms?.maidsRoom || false);
          setStorageRoom(property.rooms?.storageRoom || false);
          setLaundryRoom(property.rooms?.laundryRoom || false);
          setRoomDimensions(property.rooms?.roomDimensions || '');
          setKitchenLayout(property.rooms?.kitchenLayout || '');

          // Interior Features
          setFlooringType(property.interiorFeatures?.flooringType || '');
          setHeating(property.interiorFeatures?.heating || '');
          setCoolingAcType(property.interiorFeatures?.coolingAcType || '');
          setKitchenAppliances(property.interiorFeatures?.kitchenAppliances || false);
          setWaterHeater(property.interiorFeatures?.waterHeater || false);
          setSmartHomeFeatures(property.interiorFeatures?.smartHomeFeatures || false);
          setInternetAvailability(property.interiorFeatures?.internetAvailability || false);
          setBuiltInWardrobes(property.interiorFeatures?.builtInWardrobes || false);
          setWalkInCloset(property.interiorFeatures?.walkInCloset || false);

          // Exterior Features
          setParkingSpaces(property.exteriorFeatures?.parkingSpaces?.toString() || '');
          setGarage(property.exteriorFeatures?.garage || false);
          setGarden(property.exteriorFeatures?.garden || false);
          setPrivatePool(property.exteriorFeatures?.privatePool || false);
          setBalconyTerraceDetails(property.exteriorFeatures?.balconyTerraceDetails || '');
          setViewType(property.exteriorFeatures?.viewType || '');
          setRoofAccess(property.exteriorFeatures?.roofAccess || false);
          setFenceWall(property.exteriorFeatures?.fenceWall || false);

          // Building Features
          setElevator(property.buildingFeatures?.elevator || false);
          setSecurityConcierge(property.buildingFeatures?.securityConcierge || false);
          setCctv(property.buildingFeatures?.cctv || false);
          setGym(property.buildingFeatures?.gym || false);
          setSharedPool(property.buildingFeatures?.sharedPool || false);
          setVisitorParking(property.buildingFeatures?.visitorParking || false);
          setWheelchairAccessible(property.buildingFeatures?.wheelchairAccessible || false);
          setPetsAllowed(property.buildingFeatures?.petsAllowed || false);

          // Community
          setNearbySchools(property.community?.nearbySchools || false);
          setNearbyHospital(property.community?.nearbyHospital || false);
          setNearbyMetroPublicTransport(property.community?.nearbyMetroPublicTransport || false);
          setNearbyShoppingMall(property.community?.nearbyShoppingMall || false);
          setNearbyPark(property.community?.nearbyPark || false);
          setNearbyAirport(property.community?.nearbyAirport || false);
          setNoiseLevel(property.community?.noiseLevel || '');

          // Utilities
          setWaterSource(property.utilities?.waterSource || '');
          setElectricityProvider(property.utilities?.electricityProvider || '');
          setGasAvailability(property.utilities?.gasAvailability || false);
          setSewageSystem(property.utilities?.sewageSystem || false);
          setInternetOptions(property.utilities?.internetOptions || '');
          setHoaCondoFees(property.utilities?.hoaCondoFees?.toString() || '');
          setPropertyTax(property.utilities?.propertyTax?.toString() || '');

          // Legal
          setOwnershipType(property.legal?.ownershipType || '');
          setTitleDeedAvailable(property.legal?.titleDeedAvailable || false);
          setMortgageAllowed(property.legal?.mortgageAllowed || false);
          setDeveloperName(property.legal?.developerName || '');
          setProjectName(property.legal?.projectName || '');
          setCompletionDate(property.legal?.completionDate?.split('T')[0] || '');
          setBookingFee(property.legal?.paymentPlan?.bookingFee?.toString() || '');
          setDownPayment(property.legal?.paymentPlan?.downPayment?.toString() || '');
          setMilestones(property.legal?.paymentPlan?.milestones || '');

          // Advanced
          setEnergyEfficiencyRating(property.advanced?.energyEfficiencyRating || '');
          setSolarPanelAvailability(property.advanced?.solarPanelAvailability || false);
          setSmartLocks(property.advanced?.smartLocks || false);
          setEvChargingStation(property.advanced?.evChargingStation || false);
          setEarthquakeResistanceRating(property.advanced?.earthquakeResistanceRating || '');
        }
      } catch (err) {
        console.error(err);
  ...
  return (
    <form onSubmit={handleUpdate} style={{ maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      ...
      <button type="submit" style={{ padding: '15px 30px', fontSize: '16px', cursor: 'pointer' }}>Update Property</button>
    </form>
  );
};
